...
6) MAIN MENU:
   ...
   GAME MENU (ESP DINO , FLAPPY BIRD 🐦)

   WEB MENU

   TIME MENU (STOPWATCH ⏱ , TIMER ⏲)
...
Просто перейдите по ссылке , подключите плату к ПК удерживая кнопку boot (отпустить кнопку можно сразу после подключения платы
к ПК) и нажмите CONNECT , после выберите COM порт и нажмите подключение . После всех проделанных 
